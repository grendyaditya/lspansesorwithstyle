...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        if ($inventory) {
            ...
            $deleteStmt = $conn->prepare("DELETE FROM inventory WHERE id = :id");
...
$stmt = $conn->prepare("SELECT i.id, i.jenis_barang, i.kuantitas_stok, i.barcode, 
                    ...
                    <?php if ($inventories): ?>
                        <?php foreach ($inventories as $index => $inventory): ?>
                            ...
                            <tr class="<?= $is_out_of_stock ? 'table-danger' : ''; ?>">
                                ...
                                <td><?= htmlspecialchars($inventory['barcode']); ?></td>
                                ...
                                <td>
                                    ...
                                    <a href="?delete=<?= $inventory['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a>
                                </td>
                            </tr>
